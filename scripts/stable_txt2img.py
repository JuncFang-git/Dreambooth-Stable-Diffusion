            ...
            with model.ema_scope():
                ...
                if not opt.skip_grid:
                    ...
                    for i in range(grid.size(0)):
                        if len(opt.prompt) < 200:
                            save_image(grid[i, :, :, :], os.path.join(outpath,opt.prompt+'_{}.png'.format(i)))
                        else:
                            save_image(grid[i, :, :, :], os.path.join(outpath,opt.prompt[:200]+'_{}.png'.format(i)))
                    ...
                    if len(opt.prompt) < 200:
                        ...
                        Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, f'{prompt[:200].replace(" ", "-")}-{grid_count:04}.jpg'))
                    grid_count += 1
